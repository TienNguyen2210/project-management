...
const  Sidebar = () => {
    ...
    const sidebarClassName = `fixed flex flex-col h-full justify-between shadow-xl
        transition-all duration-300 ease-in-out z-40 dark:bg-black overflow-y-auto
        ...
        <div className='flex h-[100%] w-full flex-col justify-start'>
            ...
            <button onClick={() => setShowProjects(!showProjects)}
                className = "flex w-full items-center justify-between px-8 py-3 text-gray-500">
                    <span className='text-gray-800 dark:text-gray-100'>Projects</span>
                    {showProjects ? <ChevronUp className='h-5 w-5 text-gray-800 dark:text-gray-100' /> 
            ...
            <button onClick={() => setShowPriority(!showPriority)}
                className = "flex w-full items-center justify-between px-8 py-3 text-gray-500">
                    <span className='text-gray-800 dark:text-gray-100'>Priority</span>
                    {showPriority ? <ChevronUp className='h-5 w-5 text-gray-800 dark:text-gray-100' /> 
                                    : <ChevronDown className='h-5 w-5 text-gray-800 dark:text-gray-100' />}
            </button>
            
            {showPriority && (
                <>
                    <SidebarLink href="/priority/urgent" icon={AlertCircle} label="Urgents" />
